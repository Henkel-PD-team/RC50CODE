...
uint16_t footswitch_Status_Counter[2] ={0,0};
POWER_STATUS m_Last_footswitch_Status[2] ={POWER_OFF,POWER_OFF};
//POWER_STATUS footswitch_Status[2] ={POWER_OFF,POWER_OFF};
...
extern uint32_t id[4];
extern uint8_t mcuidbuffer[25];
float temptimer[2] ={0,0};
uint8_t  continueflag = 0;
uint16_t temp_lowercounter = 0;
uint8_t tempch = 0;
uint8_t conti_flag = 0;
uint8_t same_conti_flag = 0;
uint8_t last_conti_flag_1 = 0;
uint8_t last_conti_flag_2 = 0; 
/**
  ******************************************************************************
  *                              定义变量
  ******************************************************************************
			...
			switch(Sys_RT_Status.CH_Dispensing_Status[CHx].CH_Dispensing_Step)
			{
				...
				case DISPENSING_STATUS:
				{
					if(Sys_RT_Status.CH_Dispensing_Status[CHx].DISPENSING_COUNT_DOWN>0)
					{
						...
						Sys_Params.Channel_Param[CHx].Timer = (float)Sys_RT_Status.CH_Dispensing_Status[CHx].DISPENSING_COUNT_DOWN/100;
					...
					{ 
						...
						Sys_Params.Channel_Param[CHx].Timer = temptimer[CHx];
						...
						)
						{
								...
								{
									if(Sys_Params.Channel_Link == POWER_ON&&Sys_RT_Status.Lower_Empty_Counter==1)
									{
										Sys_RT_Status.Lower_Empty_Counter = 0;
									}
									else
									{
										Sys_RT_Status.Lower_Empty_Counter--;   //20240801
									}
...
//						}
					#endif
					if(Sys_Params.Channel_Link == POWER_ON)     //连接点胶
					{
						CHx_Manual_Dispensing[CHx]= CHx_Manual_Dispensing[CH1];
						if(DISPENSING_IO_STATUS(CH1)== POWER_ON)
						{
							GPIO_IN[DI_FOOT_SWITCH_2].GPIO_Value = POWER_ON;
							GPIO_IN[DI_CHANNEL_SWITCH_2].GPIO_Value = POWER_ON;
							GPIO_IN[DI_FOOT_SWITCH_1].GPIO_Value = POWER_ON;
							GPIO_IN[DI_CHANNEL_SWITCH_1].GPIO_Value = POWER_ON;
						}
						else
						{
							GPIO_IN[DI_FOOT_SWITCH_2].GPIO_Value = POWER_OFF;
							GPIO_IN[DI_CHANNEL_SWITCH_2].GPIO_Value = POWER_OFF;
							GPIO_IN[DI_FOOT_SWITCH_1].GPIO_Value = POWER_OFF;
							GPIO_IN[DI_CHANNEL_SWITCH_1].GPIO_Value = POWER_OFF;
						}

					}
					#if(1)
					{
						 switch(Sys_Params.Channel_Param[CHx].Run_Mode_Params)     //CH1  TIMER   CH2 CONTINUE
						 {
							 ...
							 {								 								
								 ...
								 if((DISPENSING_IO_STATUS(CHx)== POWER_ON||CHx_Manual_Dispensing[CHx]== POWER_ON)
									 &&Sys_RT_Status.Cartridge_Status!=EMPTY)			 //料没空时触发
								 { 
									 
									 if(Sys_RT_Status.Cartridge_Status==LOWER)                  //如果料快空了
									 {
								 ...
								 {	
									 if(Sys_Params.Channel_Link == POWER_ON)
									 {
										 if(last_conti_flag_1 ==1)
										 {
											 if(Sys_RT_Status.Lower_Empty_Counter>=2)
											 {
												 Sys_RT_Status.Lower_Empty_Counter = Sys_RT_Status.Lower_Empty_Counter-2;
												 last_conti_flag_1 = 0;
											 }
											 else
											 {
									 ...
									 {										 										
											if(last_conti_flag_1==1&CHx==CH1)
											{
												if(Sys_RT_Status.Lower_Empty_Counter>=1)
												{
													Sys_RT_Status.Lower_Empty_Counter--;
												}
												else
											...
											if(last_conti_flag_2==1&CHx==CH2)
											{
												if(Sys_RT_Status.Lower_Empty_Counter>=1)
												{
													Sys_RT_Status.Lower_Empty_Counter--;
							 ...
							 case TIMER:
							 {
								 switch(Sys_RT_Status.CH_Dispensing_Status[CHx].CH_Dispensing_Step)
								 {
									 case DISPENSING_STOP://=0,
									 {
										 ...
										 {
											 if(Sys_RT_Status.Cartridge_Status==LOWER)           //lowlevel点胶
											 {
												 if(Sys_Params.LowSensor_Enable==POWER_ON)
												 {
													 ...
													 temptimer[CHx] = Sys_Params.Channel_Param[CHx].Timer;
...
{ 
		...
		while (1) 
		{
			...
			{
				if(Sys_RT_Status.Lower_Empty_Counter>0&&Sys_Params.LowSensor_Enable==POWER_ON)   //开始低液位传感悟
				{
					Sys_RT_Status.Cartridge_Status=LOWER;
	...
	while (1)
	{
		...
		if (Sys_Params.IOT_Params.Commication_Mode != WIFI_MODE || (m_WIFI_Connect_Mark == 1 && Sys_Params.IOT_Params.Commication_Mode == WIFI_MODE))
		{
			...
			if (communication_back_poll() == 1)
			{
				...
				char *token = strtok(Sys_RT_Status.RT_IOT_Status.Comm_Status_Data.p_in_u8, " ");    //YOLANDA
				if (token != NULL)
				{
...
*/ //
					...
					else if (strcmp(token, "S_SYS_STATUS") == 0)
					{
						sprintf((char *)Respond, "{\"S_SYS_STATUS\": {\"System_Status\":%s}}", Sys_RT_Status.System_Status == 0 ? "\"READY\"" : Sys_RT_Status.System_Status == 1 ? "\"DISPENSING\""
																																															  : Sys_RT_Status.System_Status == 2 ? "\"STOP\""
																																															   : "\"ALM\"");
						_Recv_Finish = 1;
					}
					...
					else if (strcmp(token, "W_LOW_LEVEL_EN") == 0)
					{
						...
						if (token != NULL)
						{
							...
							SaveSystemParams();
							sprintf((char *)Respond, "{\"W_LOW_LEVEL_EN\": {\"LOW_LEVEL_EN\":%s}}", Sys_Params.LowSensor_Enable == POWER_ON ? "true" : "false");
							_Recv_Finish = 1;
						}
					}
					...
					*/
					...
					else if (strcmp(token, "R_PDT") == 0)
					{
						sprintf((char *)Respond, "{\"R_PDT\": {\"PDT\":\"%s\"}}", PDT);
						_Recv_Finish = 1;
					}
					...
					else if (strcmp(token, "R_CH1_TIMER") == 0)
					{
						...
						/*
						 */
						sprintf((char *)Respond, "{\"R_CH1_TIMER\": {\"CH1_TIMER\":%.2f}}", Sys_Params.Channel_Param[CH1].Timer);
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_CH2_TIMER") == 0)
					{
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						sprintf((char *)Respond, "{\"R_CH2_TIMER\": {\"CH2_TIMER\":%.2f}}", Sys_Params.Channel_Param[CH2].Timer);
						_Recv_Finish = 1;
					}
					...
					{
						sprintf((char *)Respond, "{\"R_PRESSURE_UNIT\": {\"PRESSURE_UNIT\":%s}}", Sys_Params.Pressure_Param.Pressure_Unit == BAR ? "\"BAR\"" : "\"PSI\"");
						_Recv_Finish = 1;
					}
					...
					else if (strcmp(token, "R_LOW_LEVEL_EN") == 0)
					{
						...
						sprintf((char *)Respond, "{\"R_LOW_LEVEL_EN\": {\"LOW_LEVEL_EN\":%s}}", Sys_Params.LowSensor_Enable == POWER_ON ? "true" : "false");
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_CH1_PURGE") == 0)
					{
						...
						//									STOP=2,
						//									ALM=3,
						sprintf((char *)Respond, "{\"R_CH1_PURGE\": {\"CH1_PURGE\":%s}}", Sys_RT_Status.CH_Manuan_Run_Mode[CH1] == POWER_ON ? "true" : "false");
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_CH2_PURGE") == 0)
					{
						sprintf((char *)Respond, "{\"R_CH2_PURGE\": {\"CH2_PURGE\":%s}}", Sys_RT_Status.CH_Manuan_Run_Mode[CH2] == POWER_ON ? "true" : "false");
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_CH1_DISPENSING") == 0)
					{
						sprintf((char *)Respond, "{\"R_CH1_DISPENSING\": {\"CH1_DISPENSING\":%s}}", (Sys_RT_Status.CH_Status[CH1]==DISPENSING) ? "true" : "false");
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_CH2_DISPENSING") == 0)
					{
						sprintf((char *)Respond, "{\"R_CH2_DISPENSING\": {\"CH2_DISPENSING\":%s}}", (Sys_RT_Status.CH_Status[CH2]==DISPENSING) ? "true" : "false");
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_PRESSURE_STATUS") == 0)
					{
						...
						sprintf((char *)Respond, "{\"R_PRESSURE_STATUS\": {\"PRESSURE_STATUS\":%s}}", Sys_RT_Status.System_Run_Mode == POWER_ON ? "true" : "false");
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						_Recv_Finish = 1;
					}
					else if (strcmp(token, "R_LINK_STATUS") == 0)
					{

						sprintf((char *)Respond, "{\"R_LINK_STATUS\": {\"LINK_STATUS\":%s}}", Sys_Params.Channel_Link == POWER_ON ? "true" : "false");
						// sprintf((char *)Respond,"PRESSURE %2f",Sys_RT_Status.RT_Pressure);
						_Recv_Finish = 1;
					}
...
{  
		...
		{  
			  ...
			  if(CHECK_RUN_STOP==POWER_ON && m_Last_DI_Run_Stop_Status==POWER_OFF && m_Last_DI_Run_Stop_Status_Counter<=10)
				...
				for(int CHx=CH1;CHx<=CH2;CHx++)
				{
					if(DISPENSING_IO_STATUS(CHx)== POWER_ON&&m_Last_footswitch_Status[CHx]==POWER_OFF&&footswitch_Status_Counter[CHx]<=10)
					{
						footswitch_Status_Counter[CHx]++;
						if(footswitch_Status_Counter[CHx]>=8)
						{
							if(Sys_Params.Channel_Link == POWER_ON)
							{
								if(DISPENSING_IO_STATUS(CH1)== POWER_ON)
								{
									...
									if(Sys_RT_Status.Lower_Empty_Counter>1)
									{
										//Sys_RT_Status.Lower_Empty_Counter = Sys_RT_Status.Lower_Empty_Counter -2; //20240801
									}
									else
									{
										//Sys_RT_Status.Lower_Empty_Counter = 0;
...
//									Sys_RT_Status.Lower_Empty_Counter --;   //20240801
							...
							m_Last_footswitch_Status[CHx] = DISPENSING_IO_STATUS(CHx);
							footswitch_Status_Counter[CHx] = 0;
						}
