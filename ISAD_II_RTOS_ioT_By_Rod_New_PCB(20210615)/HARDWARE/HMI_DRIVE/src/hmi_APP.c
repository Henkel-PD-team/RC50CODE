...
POWER_STATUS Pressure_Display_Enable=POWER_OFF;    //压力报警
...
void NotifyScreen(uint16_t screen_id)
{
	...
	{ 
		...
		{ 
			...
			SetTextValue2Float(current_screen_id,10,Sys_Params.Channel_Param[0].Timer);
			SetTextValue2Float(current_screen_id,13,Sys_Params.Channel_Param[1].Timer);
...
//			SetTextValueFloat(current_screen_id,27,Sys_Params.Channel_Param[1].Tail_Time);
			...
			{
				SetButtonValue(current_screen_id,31,1);
				//SetButtonValue(current_screen_id,35,0);
			}
			else
			{
				SetButtonValue(current_screen_id,31,0);
				//SetButtonValue(current_screen_id,35,1);
			}
...
{  
	...
	{
		if(current_screen_id==_SCREEN_PRESSURE_ALM_DLG_9)
		{
	...
	{ 
		...
		case _SCREEN_MAIN_2:
		{
			//BatchBegin(current_screen_id);
				...
				SetControlEnable(current_screen_id,5,Sys_RT_Status.System_Run_Mode);
				SetControlEnable(current_screen_id,6,Sys_RT_Status.System_Run_Mode);
				...
				if(Sys_RT_Status.System_Option_Mode==AUTORUN_MODE)
				{
					...
					/*if(Sys_RT_Status.Cartridge_Status==LOWER||Sys_RT_Status.Cartridge_Status==EMPTY)
					{
						...
						SetControlEnable(current_screen_id,25,Sys_Params.Channel_Param[0].Run_Mode_Params != CONTINUE);
						SetControlEnable(current_screen_id,26,Sys_Params.Channel_Param[1].Run_Mode_Params != CONTINUE);
...
//else
			...
			//CHx_Manual_Dispensing[CH1]
				...
				SetTextValue0Float(current_screen_id,11, Sys_RT_Status.Humidity_RT);
				
				//SetTextValue2Float(current_screen_id,28, Sys_Params.Channel_Param[0].Timer);
				//SetTextValue2Float(current_screen_id,29, Sys_Params.Channel_Param[1].Timer);	
				...
				if(Sys_Params.Channel_Link == POWER_ON)
				{
						...
						AnimationPlayFrame(current_screen_id,14, Sys_Params.Channel_Param[0].Run_Mode_Params);										
						AnimationPlayFrame(current_screen_id,15, Sys_RT_Status.CH_Status[CH1]);
						AnimationPlayFrame(current_screen_id,16, Sys_RT_Status.CH_Status[CH1]);	
						//SetTextValue2Float(current_screen_id,29, Sys_Params.Channel_Param[0].Timer);
						//SetTextValue2Float(current_screen_id,30, Sys_Params.Channel_Param[0].Timer);
						for(int CHx=CH1;CHx<=CH2;CHx++)
						{
							if(Sys_Params.Channel_Param[CHx].Run_Mode_Params==TIMER)
							{
								SetTextValue2Float(current_screen_id,(29+CHx), Sys_Params.Channel_Param[CH1].Timer);							
							}
							else
							{
								uint8_t nopro[]="--";	
								SetTextValue(current_screen_id,(29+CHx), nopro);
							}
						}
				...
				{								
						...
						{
							SetTextValue2Float(current_screen_id,29, Sys_Params.Channel_Param[CH1].Timer);							
						}
						else
						{
							uint8_t nopro[]="--";	
							SetTextValue(current_screen_id,29, nopro);
						}
						if(Sys_Params.Channel_Param[CH2].Run_Mode_Params==TIMER)
						{
							SetTextValue2Float(current_screen_id,30, Sys_Params.Channel_Param[CH2].Timer);							
						}
						else
						{
							uint8_t nopro[]="--";	
							SetTextValue(current_screen_id,30, nopro);
						}
				...
				AnimationPlayFrame(current_screen_id,17, Sys_RT_Status.Cartridge_Status);
				if(Sys_Params.LowSensor_Enable==POWER_ON)
				...
				if(Sys_Params.Channel_Link == POWER_ON)
				{
					AnimationPlayFrame(current_screen_id,28, 1);
				}
				else
				{
					AnimationPlayFrame(current_screen_id,28, 0);
				}
				SetButtonValue(current_screen_id,26,CHx_Manual_Dispensing[CH2]);
				if(Sys_Params.Channel_Param[0].Run_Mode_Params==TIMER)
				{
		...
		case _SCREEN_SETTING_3:
		{
				if(Sys_Params.Channel_Link == POWER_ON)	
				{
					Sys_Params.Channel_Param[1].Timer = Sys_Params.Channel_Param[0].Timer;
					Sys_Params.Channel_Param[1].Run_Mode_Params = Sys_Params.Channel_Param[0].Run_Mode_Params;
					SetControlEnable(current_screen_id,6,POWER_OFF);
					SetControlEnable(current_screen_id,7,POWER_OFF);
					SetControlEnable(current_screen_id,13,POWER_OFF);
					SetControlEnable(current_screen_id,18,POWER_OFF);
					SetControlEnable(current_screen_id,19,POWER_OFF);
				}
			
				AnimationPlayFrame(current_screen_id,23, Sys_RT_Status.System_Status);
					
				AnimationPlayFrame(current_screen_id,20, Sys_RT_Status.Cartridge_Status);
				if(Sys_Params.LowSensor_Enable==POWER_ON)
		...
		case _SCREEN_SYSTEMINI_5:
		{
			...
			SetTextValue(current_screen_id,5,mcuidbuffer);
...
//										Sys_RT_Status.Lower_Empty_Counter--;
		...
		case _SCREEN_SETTING_3:
		{
			switch(control_id)
			{
				...
				case 14:
				{ 
					if(state && Sys_Params.Channel_Param[0].Timer>0.05) 
					{
						Sys_Params.Channel_Param[0].Timer-=0.05;
						SetTextValue2Float(current_screen_id,10,Sys_Params.Channel_Param[0].Timer);
					}
					break;
				}
				...
				case 18:
				{ 
					if(state && Sys_Params.Channel_Param[1].Timer>0.05) 
					{
						Sys_Params.Channel_Param[1].Timer-=0.05;
						SetTextValue2Float(current_screen_id,13,Sys_Params.Channel_Param[1].Timer);
					}
					break;
				}
				...
				case 31:
				{
					if(state) 
					{
						Sys_Params.Channel_Link = POWER_ON;		
						SetControlEnable(current_screen_id,6,0);
						SetControlEnable(current_screen_id,7,0);
						SetControlEnable(current_screen_id,13,0);
						SetControlEnable(current_screen_id,18,0);
						SetControlEnable(current_screen_id,19,0);				
					...
					{
						Sys_Params.Channel_Link = POWER_OFF;		
						SetControlEnable(current_screen_id,6,1);
						SetControlEnable(current_screen_id,7,1);
						SetControlEnable(current_screen_id,13,1);
						SetControlEnable(current_screen_id,18,1);
						SetControlEnable(current_screen_id,19,1);
					}
					SetButtonValue(current_screen_id,31,Sys_Params.Channel_Link == POWER_ON);
					//SetButtonValue(current_screen_id,35,Sys_Params.Channel_Link != POWER_ON);					
