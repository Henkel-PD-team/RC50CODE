...
void EXTIX_Init(void)
{
	...
	for(int i=0;i<DI_EXTI_LEN;i++)
	{
		...
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = (i+16);//抢占优先级3
...
void EXTI_IRQHandler(void)
{
	uint32_t EXTI_LineY = EXTI->PR;       //取读是那个引脚的中断 
	if(EXTI_LineY == DI_Exti_List[0].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
				Dispensing_Logic();
				EXTI_ClearITPendingBit(EXTI_LineY);//清除LINE上的中断标志位 
			}
	}
	if(EXTI_LineY == DI_Exti_List[1].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
	...
	if(EXTI_LineY == DI_Exti_List[2].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
	...
	if(EXTI_LineY == DI_Exti_List[3].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
	...
	if(EXTI_LineY == DI_Exti_List[4].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
	...
	if(EXTI_LineY == DI_Exti_List[5].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
	...
	if(EXTI_LineY == DI_Exti_List[6].EXTI_LineX )
	{
			if(EXTI_GetITStatus(EXTI_LineY) == SET)
			{   
				Refresh_GPIO_Value();	 
